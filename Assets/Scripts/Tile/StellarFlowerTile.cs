...
[CreateAssetMenu(fileName = "StellarFlowerTile", menuName = "ScriptableObjects/Tiles/StellarFlowerTile", order = 16)]
public class StellarFlowerTile : BreakableBrickTile {
    ...
    public override bool Interact(MonoBehaviour interacter, InteractionDirection direction, Vector3 worldLocation) {
        ...
        if ((interacter is PlayerController) || (interacter is KoopaWalk koopa && koopa.previousHolder != null)) {
            ...
            spawnResult = player.state <= Enums.PowerupState.Small ? "Mushroom" : "StellarFlower";
        }
