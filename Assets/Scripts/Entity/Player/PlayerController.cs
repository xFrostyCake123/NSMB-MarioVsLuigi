...
public class PlayerController : MonoBehaviourPun, IFreezableEntity, ICustomSerializeView, IOnPhotonViewPreNetDestroy {
    ...
    private int _metalCombo;
    public int MetalCombo {
        get => metal > 0 ? _metalCombo : 0;
        set => _metalCombo = metal > 0 ? value : 0;
    }
    ...
    public GameObject onSpinner, insideWater, inTrickZone;
    ...
    bool IFreezableEntity.IsFlying => flying || propeller || gliding; //doesn't work consistently?
    ...
    public void Serialize(List<byte> buffer) {
        ...
        SerializationUtils.PackToShort(out short flags, running, jumpHeld, crouching, groundpound,
                ...
                wallSlideRight, invincible > 0, metal > 0, propellerSpinTimer > 0, wallJumpTimer > 0);
        SerializationUtils.PackToByte(out byte flags2, turnaround, propeller, squirrel);
    ...
    public void UpdateGameState() {
        ...
        UpdateGameStateVariable(Enums.NetPlayerGameState.Coins, points);
    ...
    public void OnCollisionStay2D(Collision2D collision) {
        ...
        case "Player": {
            ...
            Utils.GetCustomProperty(Enums.NetRoomProperties.FriendlyFire, out int friendlyType);
            ...
            foreach (ContactPoint2D contact in contacts) {
                ...
                if (other.invincible > 0) {
                    ...
                    if (invincible > 0) {
                        ...
                        if (other.cantDoKnockback <= 0)
                            photonView.RPC(nameof(Knockback), RpcTarget.All, otherObj.transform.position.x > body.position.x, (isTeammate && !dropStars) ? 0 : 1, true, otherView.ViewID);
                        other.photonView.RPC(nameof(Knockback), RpcTarget.All, otherObj.transform.position.x < body.position.x, (isTeammate && !dropStars) ? 0 : 1, true, photonView.ViewID);
                    }
                    return;
                }
                ...
                if (metal > 0) {
                    ...
                    otherView.RPC(nameof(Knockback), RpcTarget.All, false);
                    body.velocity = previousFrameVelocity;
                    return;
                }
                ...
                if (inShell) {
                    ...
                    if (!otherAbove) {
                        ...
                        if (other.inShell) {
                            //collide with both
                            otherView.RPC(nameof(Knockback), RpcTarget.All, otherObj.transform.position.x < body.position.x, (isTeammate && !dropStars) ? 0 : 1, true, photonView.ViewID);
                            photonView.RPC(nameof(Knockback), RpcTarget.All, otherObj.transform.position.x > body.position.x, (isTeammate && !dropStars) ? 0 : 1, true, otherView.ViewID);
                        } else {
                            otherView.RPC(nameof(Powerdown), RpcTarget.All, false);
                        }
                ...
                if (above) {
                    ...
                    if (state == Enums.PowerupState.MiniMushroom && other.state != Enums.PowerupState.MiniMushroom) {
                        ...
                        if (groundpounded) {
                            otherView.RPC(nameof(Knockback), RpcTarget.All, otherObj.transform.position.x < body.position.x, (isTeammate && !dropStars) ? 0 : 1, false, photonView.ViewID);
                            groundpound = false;
                            bounce = true;
                        } else {
                            photonView.RPC(nameof(PlaySound), RpcTarget.All, Enums.Sounds.Enemy_Generic_Stomp);
                        ...
                        otherView.RPC(nameof(Knockback), RpcTarget.All, otherObj.transform.position.x > body.position.x, (isTeammate && !dropStars) ? 0 : 3, false, photonView.ViewID);
                        bounce = false;
                    } else {
    ...
    public void OnTriggerEnter2D(Collider2D collider) {
        ...
        case "Fireball": {
            ...
            bool isActualTeammate = team == fireball.player.team && inTeam && friendly == 0;
            float dot = Vector2.Dot((body.position - fireball.body.position).normalized, Vector2.up);
            bool above = dot > 0.7f;
            if (fireball.isFlowerSpring) {
                Animator flowerAnim = fireball.GetComponent<Animator>();
                if (fireball.photonView.IsMine || (isTeammate && inTeam && friendly == 0)) {
                    bloomSpring = true;
                    body.velocity = new Vector2(body.velocity.x, 5);
                    flowerAnim.SetTrigger("boing");
                    return;
                } else {
                    bool rightSide = fireball.transform.position.x > transform.position.x;
                    bloomSpring = true;
                    if (fireball.bloomingStart > 0 && (hitInvincibilityCounter <= 0 || invincible <= 0 || state != Enums.PowerupState.MegaMushroom || !knockback)) {
                        photonView.RPC(nameof(Knockback), RpcTarget.All, rightSide, (isTeammate && !dropStars) ? 0 : 1, true, fireball.photonView.ViewID);
                        body.velocity = new Vector2(rightSide ? -3 : 3, 3);
                    } else {
            ...
            if (state == Enums.PowerupState.BlueShell && (inShell || crouching || groundpound)) {
                ...
                if (fireball.isStarball) {
                    photonView.RPC(nameof(Knockback), RpcTarget.All, fireball.left, 0, true, fireball.photonView.ViewID);
                }
                return;
            }
            if (fireball.isStarball) {
    ...
    protected void OnTriggerStay2D(Collider2D collider) {
        ...
        case "PointsBalloon": {
            photonView.RPC(nameof(AttemptCollectBalloon), RpcTarget.AllViaServer, obj.GetPhotonView().ViewID, new Vector2(obj.transform.position.x, collider.transform.position.y));
            if (combo > 3) {
                Enums.Sounds comboSound = combo switch {
                    int n when n > 15 => Enums.Sounds.UI_PointsCombo_3,
                    int n when n > 11 => Enums.Sounds.UI_PointsCombo_2, 
                    int n when n > 3 => Enums.Sounds.UI_PointsCombo_1,
                    _ => Enums.Sounds.UI_PointsCombo_1
                };
    ...
    public void OnJump(InputAction.CallbackContext context) {
        ...
        if (ridingWave && surfWave) {
            ridingWave = false; 
            bounce = true;
            body.velocity = new Vector2(5 * (surfWave.GetComponent<FireballMover>().left ? -1 : 1), body.velocity.y);
        }
    }
    ...
    private void ActivatePowerupAction() {
        ...
        case Enums.PowerupState.MagmaFlower: {
            ...
            int count = 0;
            int limit = state == Enums.PowerupState.StellarFlower ? 3 : 6;
            foreach (FireballMover existingFire in FindObjectsOfType<FireballMover>()) {
                if (existingFire.photonView.IsMine && ++count >= limit)
                    return;
            }
            if (state == Enums.PowerupState.StellarFlower) {
                if (count <= 1) {
                    fireballTimer = 2f;
        ...
        case Enums.PowerupState.CosmoShroom: {
            if (groundpound || (flying && drill) || starspin > 0 || crouching || sliding || wallJumpTimer > 0) 
                return;
        ...
        case Enums.PowerupState.TideFlower: {
            if (groundpound || (flying && drill) || propeller || flying || sliding || wallJumpTimer > 0)
                return;
            
            WaterBomb();
            break;
        }
        case Enums.PowerupState.BloomFlower: {
            if (groundpound || (flying && drill) || propeller || flying || sliding || wallJumpTimer > 0)
                return;
    ...
    private void ActivateSecondaryPowerupAction() {
        ...
        case Enums.PowerupState.BloomFlower: {
            ...
            photonView.RPC(nameof(BloomingSpringLaunch), RpcTarget.All);
            break;
        }
        }
    }
    ...
    public void SquirrelJump() {
        ...
        RaycastHit2D[] hits = Physics2D.BoxCastAll(transform.position + new Vector3(0,0.25f), new Vector2(3.5f, 1.5f), 0.15f * (state == Enums.PowerupState.MegaMushroom ? 2.5f : 1), Vector2.zero, Layers.MaskAnyGround);
        foreach (RaycastHit2D hit in hits) {
            ...
            case "Fireball": {
                FireballMover otherFireballs = obj.GetComponent<FireballMover>();
                if (!otherFireballs.photonView.IsMine)
                    return;

                otherFireballs.photonView.RPC(nameof(KillableEntity.Kill), RpcTarget.All);
                break;
            }
            }
        }
    ...
    [PunRPC]
    public void CosmicSpin() {
        if (usedStarSpinThisJump || onStarspinCooldown > 0)
            return;
        ...
        if (onGround)
            ...
            body.velocity = new Vector2(body.velocity.x, propellerLaunchVelocity);
            starspinTimer = 0.45f;
            PlaySound(Enums.Sounds.Player_Voice_StarSpin);
    ...
    public void WaterBomb() {
        ...
        if (!bombDeployed) {
            if (wallSlideLeft || wallSlideRight || groundpound || triplejump || flying || ridingWave || drill || crouching || sliding)
                return;
            ...
            if (state == Enums.PowerupState.TideFlower) {
                canShootProjectile = false;
                if (fireballTimer <= 0) {
                    fireballTimer = 1.5f;
                } else {
    ...
    public void WaveRide() { // funni..
        string wave = "TidalWave";
        Enums.Sounds rideSound = Enums.Sounds.Powerup_WaterShoot;
        ...
        Vector2 pos = body.position + new Vector2(facingRight ^ animator.GetCurrentAnimatorStateInfo(0).IsName("turnaround") ? 0.5f : -0.5f, 0.3f);
            if (Utils.IsTileSolidAtWorldLocation(pos)) {
                photonView.RPC(nameof(SpawnParticle), RpcTarget.All, $"Prefabs/Particle/{wave}Wall", pos);
            } else {
            surfWave = PhotonNetwork.Instantiate($"Prefabs/{wave}", pos, Quaternion.identity, 0, new object[] { !facingRight ^ animator.GetCurrentAnimatorStateInfo(0).IsName("turnaround"), body.velocity.x });
            }
            photonView.RPC(nameof(PlaySound), RpcTarget.All, rideSound);
    ...
    public void FlowerSpring() {
        bool maro = Mario;
        bool loogi = Luigi;
        bool upInput = joystick.y > 0.5;
        string projectile = "FlowerSpring";
        Enums.Sounds sound = Enums.Sounds.Powerup_SpringBounce;
        Enums.Sounds sound2 = Enums.Sounds.Powerup_SpringPop;
        if (wallSlideLeft || wallSlideRight || groundpound || triplejump || flying || drill || crouching || sliding)
            return;
        ...
        Utils.GetCustomProperty(Enums.NetRoomProperties.TeamsMatch, out bool teaming);
        Vector2 pos = body.position + new Vector2(facingRight ^ animator.GetCurrentAnimatorStateInfo(0).IsName("turnaround") ? 0.5f : -0.5f, 0f);
        PhotonNetwork.Instantiate($"Prefabs/{projectile}", pos, Quaternion.identity, 0, new object[] { !facingRight ^ animator.GetCurrentAnimatorStateInfo(0).IsName("turnaround"), body.velocity.x });
        photonView.RPC(nameof(PlaySound), RpcTarget.All, sound);
    ...
    [PunRPC]
    protected void BloomingSpringLaunch() {
        if (onSpringCooldown > 0)
            return;

        body.velocity = new Vector2(body.velocity.x, 7);
        PlaySound(Enums.Sounds.Powerup_SpringBounce);
        PlaySound(Enums.Sounds.Player_Voice_TripleJump);
        PhotonNetwork.Instantiate("Prefabs/FlowerSpring", body.position, Quaternion.identity, 0, new object[] { !facingRight ^ animator.GetCurrentAnimatorStateInfo(0).IsName("turnaround"), body.velocity.x });
        triplejump = true;
    ...
    protected void Powerup(int actor, PhotonMessageInfo info) {
        ...
        } else if (powerup.prefab == "MetalCap") {
            //metal
            if (metal <= 0)
                MetalCombo = 0;
    ...
    public void Freeze(int cube) {
        if (knockback || hitInvincibilityCounter > 0 || invincible > 0 || metal > 0 || Frozen || state == Enums.PowerupState.MegaMushroom)
            return;
    ...
    protected void CollectCoin(int coinID, int newCount, Vector2 position, PhotonMessageInfo info) {
        ...
        num.text.text = Utils.GetSymbolString((((teams && shareCoins) ? GameManager.Instance.teamController.GetTeamCoins(team) : coins) + 1).ToString(), Utils.numberSymbols);
        ...
        if (teams && shareCoins) {
            if (gm.teamController.GetTeamCoins(team) >= (gm.teamController.TeamCoinRequirement(team) - 1)) {
                gm.teamController.GrantTeamPowerup(team);
            }
        } else {
    ...
    [PunRPC]
    public void AttemptCollectBalloon(int balloonID, Vector2 particle, PhotonMessageInfo info) {
        //only the owner can request a coin, and only the master client can decide for us
        if (!PhotonNetwork.IsMasterClient)
            return;
        ...
        int newPoints = 0;
        if (balloonID != -1) {
            PhotonView balloon = PhotonView.Find(balloonID);
            if (!balloon || !balloon.IsMine || !balloon.GetComponent<SpriteRenderer>().enabled)
                return;

            PointsBalloon pb = balloon.GetComponent<PointsBalloon>();
            if (pb != null)
                newPoints = pb.points;
        }

        if (comboTimer > 0 && newPoints > 0 && combo > 1) {
            float multiplier = combo switch {
                int n when n > 20 => 1 + (combo * 0.03f),
                int n when n > 15 => 1 + (combo * 0.11f),
                int n when n > 11 => 1 + (combo * 0.09f),
                int n when n > 7 => 1 + (combo * 0.07f),
                int n when n > 3 => 1 + (combo * 0.05f),
                _ => 1 + (combo * 0.02f)
            };    
            float roundedPoints = newPoints * multiplier;
            newPoints = Mathf.RoundToInt(roundedPoints);
        }

        photonView.RPC(nameof(ObtainPoints), RpcTarget.All, balloonID, points + newPoints, particle);
    }

    [PunRPC]
    protected void ObtainPoints(int balloonID, int newPoints, Vector2 position, PhotonMessageInfo info) {
        //only trust the master client
        if (!info.Sender.IsLocal && !info.Sender.IsMasterClient)
            return;
        ...
        if (balloon) {
            ...
            balloon.GetComponent<BoxCollider2D>().enabled = false;
            balloon.GetComponent<PointsBalloon>().countText.enabled = false;
            balloon.GetComponent<AudioSource>().enabled = false;
        }
        ...
        int np = 0;
        np = pb.points;
        if (comboTimer > 0 && np > 0 && combo > 1) {
            float multiplier = 1 + (combo * 0.02f);
            float roundedPoints = np * multiplier;
            np = Mathf.RoundToInt(roundedPoints);
        }
        Instantiate(Resources.Load("Prefabs/Particle/StarCollect"), position, Quaternion.identity);
        Enums.Sounds collectSound = pb.points < 0 ? Enums.Sounds.UI_Error : pb.points == 0 ? Enums.Sounds.UI_WindowClose : Enums.Sounds.World_Star_Collect_Self;
        PlaySound(collectSound);

        NumberParticle num = ((GameObject) Instantiate(Resources.Load("Prefabs/Particle/Number"), position, Quaternion.identity)).GetComponentInChildren<NumberParticle>();
        num.text.text = Utils.GetSymbolString((pb.points > 0 ? "+" : ""), Utils.numberSymbols) + Utils.GetSymbolString(np.ToString(), Utils.numberSymbols);
        num.ApplyColor(pb.points < 0 ? new Color(1f, 0f, 0f, 1f) : pb.points == 0 ? new Color(0.6f, 0.6f, 0.6f, 1f) : pb.points > 0 ? new Color(0f, 1f, 0f, 1f) : new Color(1f, 1f, 1f, 1f));

        combo++;
        comboTimer = pb.points < 0 ? 1.25f : pb.points == 0 ? 1.75f : 2.25f;
    ...
    public void SpawnBobomb() {
        ...
        Vector2 pos = body.position + new Vector2(facingRight ^ animator.GetCurrentAnimatorStateInfo(0).IsName("turnaround") ? 0.5f : -0.5f, 0.3f);
        PlaySound(sound);
        bobomb = PhotonNetwork.InstantiateRoomObject(projectile, pos + new Vector2(0, 2), Quaternion.identity);
    ...
    public Vector2 GetHitboxSize(bool crouching) {
        ...
        if (state <= Enums.PowerupState.Small || (invincible > 0 && !onGround && !crouching && !sliding && !flying && !propeller) || (metal > 0 && !onGround && !crouching && !sliding && !flying && !propeller) || groundpound) {
    ...
    void TickCounters() {
        ...
        if (!pipeEntering)
            ...
            Utils.TickTimer(ref metal, 0, delta);
    ...
    private void HandleMovement(float delta) {
        ...
        if (starspin > 0) {
            body.velocity = new Vector2(body.velocity.x, propellerLaunchVelocity - (starspin < .4f ? (1 - (starspin / .4f)) * propellerLaunchVelocity : 0));
        }
        ...
        //slow-rise check
        if (flying || propeller || (bloomSpring && body.velocity.y > 0)) {
        ...
        if (flying) {
            ...
            float htv = RunningMaxSpeed * 1.15f;
        ...
        } else if ((wallSlideLeft  || wallSlideRight) && squirrelWallSlide !> 0) {
            body.velocity = new(body.velocity.x, Mathf.Max(body.velocity.y, squirrelWallSlide > 0 ? acornWallSlideSpeed : wallslideSpeed));
